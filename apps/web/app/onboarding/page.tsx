      ...
      <div className="onboarding-layout">
        <form className="card" onSubmit={submit}>
          ...
          <div className="form-grid">
            ...
            <div className="field"><label>Sport / racing category</label><input aria-label="Sport or racing category" required value={profile.sport} onChange={(e) => setProfile({ ...profile, sport: e.target.value })} /></div>
            <div className="field"><label>Nationality</label><input aria-label="Nationality" value={profile.nationality ?? ""} onChange={(e) => setProfile({ ...profile, nationality: e.target.value })} placeholder="Your nationality" /></div>
            ...
            <div className="field"><label>Current series or programme</label><input aria-label="Current series or programme" value={profile.currentSeries} onChange={(e) => setProfile({ ...profile, currentSeries: e.target.value, targetSeries: e.target.value })} placeholder="Your current championship, tour or programme" /></div>
            ...
            <div className="field full"><label>Countries you compete in</label><input aria-label="Countries you compete in" required value={profile.competitionCountries.join(", ")} onChange={(e) => setProfile({ ...profile, competitionCountries: split(e.target.value) })} placeholder="United States, France, Japan…" /></div>
            <div className="field full"><label>Career goals</label><textarea aria-label="Career goals" value={profile.futureGoals} onChange={(e) => setProfile({ ...profile, futureGoals: e.target.value })} placeholder="GT3, WEC and Le Mans..." /></div>
            <div className="field full"><label>Achievements</label><textarea aria-label="Achievements" value={profile.achievements ?? ""} onChange={(e) => setProfile({ ...profile, achievements: e.target.value })} /></div>
            <div className="field full"><label>Personal story and differentiators</label><textarea aria-label="Personal story and differentiators" value={profile.personalStory} onChange={(e) => setProfile({ ...profile, personalStory: e.target.value })} placeholder="Why should a company remember you?" /></div>
          </div>
          ...
          <div className="form-grid">
            <div className="field full"><label>Markets you want sponsors from</label><input aria-label="Markets you want sponsors from" required value={profile.targetCountries.join(", ")} onChange={(e) => setProfile({ ...profile, targetCountries: split(e.target.value) })} /></div>
            <div className="field full"><label>Main audience countries</label><input aria-label="Main audience countries" value={profile.audienceCountries.join(", ")} onChange={(e) => setProfile({ ...profile, audienceCountries: split(e.target.value) })} placeholder="Your home and audience markets" /></div>
            <div className="field full"><label>Preferred industries</label><input aria-label="Preferred industries" required value={profile.preferredIndustries.join(", ")} onChange={(e) => setProfile({ ...profile, preferredIndustries: split(e.target.value) })} /></div>
          ...
          <div className="form-grid">
            ...
            <div className="field"><label>Daily email cap</label><input aria-label="Daily email cap" type="number" min={0} value={profile.dailyEmailLimit} onChange={(e) => setProfile({ ...profile, dailyEmailLimit: Number(e.target.value) })} /><small>0 means the user has not set a GridFlow cap.</small></div>
            <div className="field"><label>Timezone</label><input aria-label="Timezone" value={profile.timezone} onChange={(e) => setProfile({ ...profile, timezone: e.target.value })} /></div>
            <div className="field"><label>Outreach tone</label><input aria-label="Outreach tone" value={profile.tone} onChange={(e) => setProfile({ ...profile, tone: e.target.value })} /></div>
          </div>
