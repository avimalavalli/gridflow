...
export function ForgotPasswordForm() {
  ...
  return <main className="auth-shell" id="main-content"><section className="auth-card" aria-labelledby="forgot-title"><div className="auth-brand"><span>GRID</span>FLOW</div><div className="eyebrow">Account recovery</div><h1 id="forgot-title">Reset your password</h1><p>Enter the email address linked to your GridFlow account.</p><form className="auth-form" onSubmit={submit}><label>Email<input required type="email" autoComplete="email" value={email} onChange={(event)=>setEmail(event.target.value)} /></label><button className="button button-primary button-large" disabled={busy}>{busy ? "Requesting…" : "Send reset link"}</button>{message ? <div className="notice" role="status">{message}</div> : null}</form><div className="auth-footer"><Link href="/login">Back to sign in</Link></div></section></main>;
}
