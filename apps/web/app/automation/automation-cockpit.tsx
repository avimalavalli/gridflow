...
export function AutomationCockpit({ initial }: { initial: AutomationOverview }) {
  ...
  return <Shell title="Automation Cockpit">
    <PageHead eyebrow="Phase 6.3 · Automation Cockpit" title="Run the commercial system by exception" description="GridFlow watches the work, handles safe internal actions and brings you only the decisions that need human judgement." action={<button className="button button-primary" type="button" disabled={Boolean(busy)} onClick={() => void request("/run-now", { method: "POST", body: "{}" }, "Automation check completed.")}><RefreshCcw size={15} className={busy === "/run-now" ? "spin" : ""}/>Run safe check</button>} />
    ...
    {view === "TODAY" ? <>
      <section className="metrics metrics-six automation-metrics">
        ...
        <div className="metric"><span className="metric-icon"><Bot size={17}/></span><div className="metric-label">Agents active</div><div className="metric-value">{data.metrics.activeRuns}</div><div className="metric-foot">{data.metrics.agentRunsToday} runs in 24 hours</div></div>
