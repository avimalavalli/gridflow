      ...
      <div className="calibration-layout">
        ...
        <section className="calibration-card">
          ...
          {step === 0 ? <div className="form-grid">
            <div className="field"><label>Your name</label><input required value={profile.name} onChange={(event) => setProfile({ ...profile, name: event.target.value })} placeholder="Jordan Taylor"/></div>
            <div className="field"><label>Sport / racing category</label><input required value={profile.sport} onChange={(event) => setProfile({ ...profile, sport: event.target.value })} placeholder="GT racing"/></div>
            <div className="field"><label>Nationality</label><input value={profile.nationality ?? ""} onChange={(event) => setProfile({ ...profile, nationality: event.target.value })}/></div>
            <div className="field"><label>Country you are based in</label><input required value={profile.residenceCountry} onChange={(event) => setProfile({ ...profile, residenceCountry: event.target.value })}/></div>
            <div className="field"><label>Current series</label><input value={profile.currentSeries} onChange={(event) => setProfile({ ...profile, currentSeries: event.target.value, targetSeries: event.target.value })}/></div>
            <div className="field"><label>Current team</label><input value={profile.currentTeam} onChange={(event) => setProfile({ ...profile, currentTeam: event.target.value })}/></div>
            <div className="field full"><label>Current programme</label><textarea value={profile.currentProgramme} onChange={(event) => setProfile({ ...profile, currentProgramme: event.target.value })} placeholder="Season, events, test programme and immediate priorities"/></div>
            <div className="field full"><label>Countries you compete in</label><input required value={profile.competitionCountries.join(", ")} onChange={(event) => setProfile({ ...profile, competitionCountries: split(event.target.value) })} placeholder="United Kingdom, France, Germany"/><small>Separate countries with commas.</small></div>
          </div> : null}
          ...
          {step === 2 ? <div className="linkedin-studio">
            ...
            <div className="linkedin-template-grid">
              <div className="field"><div className="field-heading"><label>Sponsor-ready headline</label><button type="button" onClick={() => setProfile({ ...profile, linkedinHeadline: draftHeadline })}><Sparkles size={13}/>Use tailored draft</button></div><textarea className="linkedin-headline-input" maxLength={220} value={profile.linkedinHeadline} onChange={(event) => setProfile({ ...profile, linkedinHeadline: event.target.value })} placeholder={draftHeadline}/><small>{profile.linkedinHeadline.length}/220 characters · Keep every claim accurate.</small><button className="inline-copy" type="button" onClick={() => void copy(profile.linkedinHeadline || draftHeadline, "headline")}><Copy size={13}/>{copied === "headline" ? "Copied" : "Copy for LinkedIn"}</button></div>
              <div className="field"><div className="field-heading"><label>About section</label><button type="button" onClick={() => setProfile({ ...profile, linkedinAbout: draftAbout })}><Sparkles size={13}/>Use tailored draft</button></div><textarea className="linkedin-about-input" value={profile.linkedinAbout} onChange={(event) => setProfile({ ...profile, linkedinAbout: event.target.value })} placeholder={draftAbout}/><small>First person, evidence-led and easy to scan. Edit the draft until it sounds like you.</small><button className="inline-copy" type="button" onClick={() => void copy(profile.linkedinAbout || draftAbout, "about")}><Copy size={13}/>{copied === "about" ? "Copied" : "Copy for LinkedIn"}</button></div>
            </div>
          ...
          {step === 3 ? <div className="form-grid">
            <div className="field full"><label>Career goals</label><textarea value={profile.futureGoals} onChange={(event) => setProfile({ ...profile, futureGoals: event.target.value })} placeholder="Where are you going over the next two to three years?"/></div>
            <div className="field full"><label>Achievements</label><textarea value={profile.achievements ?? ""} onChange={(event) => setProfile({ ...profile, achievements: event.target.value })} placeholder="Results, milestones, awards and credible proof"/></div>
            <div className="field full"><label>Your story</label><textarea value={profile.personalStory} onChange={(event) => setProfile({ ...profile, personalStory: event.target.value })} placeholder="What shaped your journey and makes it commercially memorable?"/></div>
            <div className="field full"><label>Commercial differentiators</label><textarea value={profile.differentiators} onChange={(event) => setProfile({ ...profile, differentiators: event.target.value })} placeholder="Audience, access, geography, content, engineering insight, community…"/></div>
            <div className="field full"><label>Audience summary</label><textarea value={profile.audienceSummary} onChange={(event) => setProfile({ ...profile, audienceSummary: event.target.value })} placeholder="Who follows you, where they are and how you engage them"/></div>
            <div className="field full"><label>Voice and tone</label><input value={profile.tone} onChange={(event) => setProfile({ ...profile, tone: event.target.value })}/></div>
          </div> : null}

          {step === 4 ? <div className="form-grid">
            <div className="field full"><label>Target sponsor countries</label><input required value={profile.targetCountries.join(", ")} onChange={(event) => setProfile({ ...profile, targetCountries: split(event.target.value) })}/></div>
            <div className="field full"><label>Main audience countries</label><input value={profile.audienceCountries.join(", ")} onChange={(event) => setProfile({ ...profile, audienceCountries: split(event.target.value) })}/></div>
            <div className="field full"><label>Preferred industries</label><input required value={profile.preferredIndustries.join(", ")} onChange={(event) => setProfile({ ...profile, preferredIndustries: split(event.target.value) })} placeholder="Technology, engineering, finance"/></div>
            <div className="field full"><label>Industries to exclude</label><input value={profile.excludedIndustries.join(", ")} onChange={(event) => setProfile({ ...profile, excludedIndustries: split(event.target.value) })} placeholder="Gambling, tobacco"/></div>
            <div className="field"><label>Minimum useful partnership</label><div className="input-prefix"><span>{profile.currency}</span><input type="number" min={0} value={profile.sponsorshipTargetMin ?? 0} onChange={(event) => setProfile({ ...profile, sponsorshipTargetMin: Number(event.target.value) })}/></div></div>
            <div className="field"><label>Maximum target partnership</label><div className="input-prefix"><span>{profile.currency}</span><input type="number" min={0} value={profile.sponsorshipTargetMax ?? 0} onChange={(event) => setProfile({ ...profile, sponsorshipTargetMax: Number(event.target.value) })}/></div></div>
            <div className="field"><label>Currency</label><select value={profile.currency} onChange={(event) => setProfile({ ...profile, currency: event.target.value })}><option>USD</option><option>GBP</option><option>EUR</option><option>AED</option><option>INR</option></select></div>
