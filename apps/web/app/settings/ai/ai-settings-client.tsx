...
export function AiSettingsClient({ data }: { data: AiSettingsData }) {
  ...
  async function remove() {
    if (!window.confirm("Delete this organisation's encrypted Gemini key? Core AI agents will pause until another key is connected.")) return;
    setBusy(true); setMessage("");
    try {
  ...
  return (
    <div className="stack">
      <div className="grid-3">
        ...
        <article className="metric-card"><span>Gemini</span><strong>{data.gemini.connected ? "Connected" : "Required"}</strong><small>{data.gemini.model ?? "No key saved"}</small></article>
      ...
      <section className="card">
        <div className="section-header"><div><div className="eyebrow">Five-minute setup</div><h2>Connect the free Gemini key</h2><p>The key belongs to this organisation and is used only for non-web agents.</p></div><KeyRound size={22} /></div>
