  ...
  return <Shell title="Agent Runs">
    ...
    {error ? <DataUnavailable message={error} /> : <section className="card">
      {runs.length === 0 ? <div className="empty">No agent runs yet. Activate a Discovery Brief and queue Atlas.</div> :
        <div className="table-wrap"><table><thead><tr><th>Agent</th><th>Status</th><th>Created</th><th>Model</th><th>Usage</th><th>Failure</th><th>Action</th></tr></thead><tbody>{runs.map((run) => <tr key={run.id}><td><strong>{run.agentName}</strong><div className="table-sub">{run.promptVersion ?? "No prompt version"}</div></td><td><span className={`badge ${run.status === "SUCCEEDED" ? "green" : run.status === "FAILED" ? "red" : run.status === "RUNNING" ? "blue" : "amber"}`}>{run.status}</span><div className="table-sub">Retries: {run.retryCount}</div></td><td>{time(run.createdAt)}<div className="table-sub">Completed: {time(run.completedAt)}</div></td><td>{run.modelUsed ?? "Pending"}</td><td>{run.totalTokens ?? 0} tokens<div className="table-sub">${Number(run.estimatedCostUsd ?? 0).toFixed(4)}</div></td><td>{run.errorDetails ? <span title={run.errorDetails}>{run.errorCode ?? "Failed"}<div className="table-sub">{run.errorDetails.slice(0, 90)}</div></span> : "—"}</td><td>{run.status === "FAILED" ? <RetryAgentButton id={run.id} /> : <span className="table-sub">{run.status === "QUEUED" ? "Waiting for worker" : run.status === "RUNNING" ? "Processing" : "Complete"}</span>}</td></tr>)}</tbody></table></div>}
    </section>}
  </Shell>;
}
