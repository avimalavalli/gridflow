...
interface Experience { progress: { setupDismissedAt: string | null }; setup: { completed: number; total: number; steps: SetupStep[]; next: SetupStep | null } }
...
export function SetupChecklist() {
  ...
  async function dismiss(): Promise<void> {
    setHidden(true);
    await fetch("/backend/experience", {
      method: "PATCH",
      credentials: "include",
  ...
  if (!experience || hidden || experience.progress.setupDismissedAt || experience.setup.completed === experience.setup.total) return null;
  const percent = Math.round((experience.setup.completed / experience.setup.total) * 100);
  return <section className="setup-banner">
    <div className="setup-banner-head"><div><div className="eyebrow">Finish setting up GridFlow</div><h2>{experience.setup.completed} of {experience.setup.total} foundations complete</h2><p>GridFlow checks the real workspace state, so this list updates automatically as you work.</p></div><button className="icon-button" type="button" aria-label="Dismiss setup checklist" onClick={() => void dismiss()}><X size={16}/></button></div>
