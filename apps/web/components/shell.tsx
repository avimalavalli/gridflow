...
const navigation: readonly { label: string; items: readonly NavigationItem[] }[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Follow-ups", href: "/pulse", icon: TimerReset, keywords: "pulse follow ups timing reminders cadence stopped sequences" },
      { label: "Reply Inbox", href: "/sentinel", icon: MessagesSquare, keywords: "sentinel inbound replies intent classification review opt out" },
      { label: "Response Strategy", href: "/nova", icon: Sparkles, keywords: "nova reply draft strategy objection opportunity meeting recommendation" },
      { label: "Meeting Intelligence", href: "/orbit", icon: OrbitIcon, keywords: "orbit meeting preparation agenda briefing debrief notes follow up tasks" },
      { label: "Proposals", href: "/forge", icon: Hammer, keywords: "forge proposal packages pricing activation commercial approval pdf" },
      { label: "Contracts", href: "/seal", icon: FileSignature, keywords: "seal contracts signatures legal payments milestones invoices revenue" },
    ],
  },
  {
    label: "Plan and record",
    items: [
      ...
      { label: "Research Runs", href: "/agent-runs", icon: Bot, keywords: "agent atlas sage relay echo jobs errors cost" },
    ],
  },
  {
    label: "Learn",
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Intelligence Setup", href: "/settings/ai", icon: KeyRound, keywords: "ai gemini api key provider credits artificial intelligence" },
...
export function Shell({ children, title }: { children: ReactNode; title: string }) {
  ...
  const [expandedSections, setExpandedSections] = useState<Set<string>>(() => {
    const active = navigation.find((section) => section.items.some((item) => isActive(pathname, item.href)))?.label;
    return new Set(["Core workflow", ...(active && active !== "Core workflow" ? [active] : [])]);
  });
  ...
  useEffect(() => {
    const active = navigation.find((section) => section.items.some((item) => isActive(pathname, item.href)))?.label;
    if (!active) return;
    setExpandedSections((current) => current.has(active) ? current : new Set([...current, active]));
  }, [pathname]);
  ...
  const organisationType = formatLabel(auth?.activeOrganisation.organisationType, "Commercial operating system");
    ...
    <div className="app-shell">
      ...
      <aside className={`sidebar ${mobileOpen ? "sidebar-open" : ""}`}>
          ...
          {visibleNavigation.map((section) => (
            <div className="nav-section" key={section.label}>
              <button
                ...
                aria-controls={`nav-${section.label.toLowerCase().replaceAll(" ", "-")}`}
                onClick={() => toggleSection(section.label)}
              >
                <span>{section.label}</span>
                <ChevronDown size={14} />
              </button>
              <div className="nav-section-items" id={`nav-${section.label.toLowerCase().replaceAll(" ", "-")}`} hidden={!expandedSections.has(section.label)}>{section.items.map((item) => {
        ...
        <div className="sidebar-footer">
          <div className="system-chip"><ShieldCheck size={15} /><span><strong>Human controlled</strong><small>Evidence attached to decisions</small></span></div>
          <LogoutButton />
        </div>
      </aside>
      ...
      <main className="main" id="main-content" tabIndex={-1}>
        <header className="topbar">
          <div className="topbar-left">
            ...
            <div><div className="topbar-kicker">{organisationName}</div><div className="topbar-title">{title}</div></div>
          </div>
      ...
      {searchOpen ? (
        <div className="command-layer" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setSearchOpen(false); }}>
          <section className="command-palette" role="dialog" aria-modal="true" aria-label="Search GridFlow">
            <div className="command-input-row"><Search size={19} /><input ref={searchInput} maxLength={80} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search records and workspaces…" onKeyDown={(event) => { if (event.key === "Enter") { const first = recordResults[0]?.href ?? navigationResults[0]?.href; if (first) goTo(first); } }} /><button type="button" onClick={() => setSearchOpen(false)} aria-label="Close search"><X size={17} /></button></div>
            <div className="command-results">
              {query.trim().length >= 2 ? <><div className="command-section-label"><span>Commercial records</span>{searchingRecords ? <small role="status">Searching…</small> : <small>{recordResults.length} found</small>}</div>{recordResults.map((item) => {
                ...
                return <button className="command-result" type="button" key={`record:${item.kind}:${item.id}`} onClick={() => goTo(item.href)}><span className="command-icon"><Icon size={17} /></span><span><strong>{item.title}</strong><small>{formatLabel(item.kind)} · {item.subtitle}</small></span><ChevronRight size={16} /></button>;
              })}</> : null}
