...
export default async function CalendarPage() {
  ...
  return <Shell title="Calendar"><PageHead eyebrow="Commercial rhythm" title="See every meeting, task and expected close in one place" description="GridFlow turns separate records into one operating calendar so sponsor conversations never disappear between screens." action={<Link className="button button-primary" href="/meetings"><CalendarPlus size={14}/>Schedule meeting</Link>}/>{error ? <DataUnavailable message={error}/> : <CalendarClient tasks={tasks} meetings={meetings} opportunities={opportunities}/>}</Shell>;
}
